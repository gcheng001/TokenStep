...
struct ShareRhythmCardView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if TokenStepThemeRuntime.isVoyage {
                ...
                InterstellarBackdrop(role: .share, isScreenshotRendering: true)
                    .opacity(0.88)
            }
    ...
    private var header: some View {
        HStack(alignment: .top, spacing: 13) {
            ...
            VStack(alignment: .trailing, spacing: 5) {
                ...
                Text(weekdayText)
                    .font(.callout.weight(.bold))
                    .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.56) : Color.white.opacity(0.56))
            }
        }
    }
    ...
    private var hero: some View {
        VStack(spacing: 8) {
            ...
            Text(rhythm.primaryTag.shareLine)
                .font(.callout.weight(.bold))
                .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.66) : Color.white.opacity(0.66))
                .lineLimit(1)
    ...
    private var peakCapsule: some View {
        HStack(spacing: 13) {
            ...
            Text(LFormat("峰值 %@", TokenStepFormat.tokens(rhythm.peakTokens, compact: true)))
                ...
                .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.72) : Color.white.opacity(0.72))
                .lineLimit(1)
        }
    ...
    private var tokenConsole: some View {
        HStack(spacing: 14) {
            ...
            VStack(spacing: 3) {
                Text(L("昨日 Token"))
                    .font(.headline.weight(.black))
                    .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.70) : Color.white.opacity(0.70))
                Text(TokenStepFormat.tokens(day.totalTokens, compact: true))
                    .font(.system(size: 49, weight: .black, design: .rounded))
                    .foregroundStyle(
                        LinearGradient(
                            colors: [palette.accent, TokenStepThemeRuntime.isCinematic ? Color.tokenInk : Color.white],
                            startPoint: .top,
                            endPoint: .bottom
                        )
                    )
    ...
    private var footer: some View {
        ...
        .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.54) : Color.white.opacity(0.54))
        .frame(maxWidth: .infinity)
        .frame(height: 38)
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text(AgentWorkCopy.hourLabel(hour))
                .font(.caption.weight(.heavy))
                .foregroundStyle(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.48) : Color.white.opacity(0.48))
            if let symbol {
    ...
    var body: some View {
        GeometryReader { proxy in
            if let peakHour, values.indices.contains(peakHour) {
                ...
                Circle()
                    .fill(TokenStepThemeRuntime.isCinematic ? Color.tokenInk : Color.white)
    ...
    var body: some View {
        ZStack {
            ...
            RhythmGridShape(columns: 11, rows: 14)
                .stroke(TokenStepThemeRuntime.isCinematic ? Color.tokenInk.opacity(0.026) : Color.white.opacity(0.026), lineWidth: 1)
        }
    }
}
